...
// from ebpf.Collection to be used later for TestLoad* tests to verify
// loaded programs and maps.
//
// We can't just iterate all load programs and maps for 2 reasons:
//   - we could race with others loading bpf programs
//   - we get limited name, which we usually cross
//
...
//     so we filter out unused programs/maps with
//
//     load.LC = filterLoadedCollection(lc)
//
// This was we have only programs/maps realted to our test and with
// original long names.
//
...
func copyLoadedCollection(lc *LoadedCollection, coll *ebpf.Collection) error {
	if lc == nil || coll == nil {
		return nil
	}
	...
	for name, p := range coll.Programs {
		...
		if !ok {
			return fmt.Errorf("failed to get map ids")
		}
